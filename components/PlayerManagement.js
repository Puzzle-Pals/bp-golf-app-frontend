...
export default function PlayerManagement() {
  ...
  async function fetchPlayers() {
    try {
      setPlayers(await adminGetPlayers());
      setError('');
    } catch (err) {
  ...
  async function handleAddOrUpdate(e) {
    ...
    try {
      if (editing) {
        await adminUpdatePlayer({ ...form, id: editing });
      } else {
        await adminAddPlayer(form);
      }
      ...
      <ul>
        {players.map(p => (
          <li key={p.id}>
            ...
            <button className="btn btn-link btn-sm" onClick={() => handleEdit(p)}>
              Edit
            </button>
            <button className="btn btn-link btn-sm text-danger" onClick={() => handleDelete(p.id)}>
              Delete
            </button>
          </li>
        ))}
